...
| overrideMICommands | boolean | Both | When true, rewrites standard MI thread commands (-thread-info, -thread-list-ids, -thread-select, -stack-list-frames) and the --thread option to their -override- equivalents. Use with a GDB Python extension (e.g. zephyr_gdb.py) that implements the override commands for RTOS thread awareness. |
